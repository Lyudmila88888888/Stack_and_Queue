...
class Queue {
private:
	ANY data[max_size]; //массив с элементом
	size_t size;
public:
	Queue() : size(0) {} //конструктор по умолчанию
	void put(ANY value); //put метод позволяет добавить элемент
	ANY get();
	ANY look();// посмотреть сколько элементов в очереди
	bool isEmpty();
...
void Queue<ANY, max_size>::put(ANY value) {
	...
	data[size++] = value; // текущий элемент  заполняем значением и потом переходим к следующему
}
